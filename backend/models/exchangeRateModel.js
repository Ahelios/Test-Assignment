...
const getAllRates = async () => {
  try {
    const [rows] = await db.query('SELECT * FROM rates ORDER BY updated_at DESC');
    return rows;
  } catch (error) {
...
const updateRates = async (baseCurrency, targetCurrency, rate) => {
  try {
    const [result] = await db.query(
      ...
      'ON DUPLICATE KEY UPDATE exchange_rate = ?, updated_at = CURRENT_TIMESTAMP',
      [baseCurrency, targetCurrency, rate, rate]
    );
    return result;
  } catch (error) {
